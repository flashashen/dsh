...
class CmdProto():
    ...
    def __str__(self):
        str =  "CmdProto {}. {}".format(self.name, self.help)
        if self._cmd_list:
            str += '. Children'
            for cmd in self._cmd_list:
                str += 'CmdProto {}'.format(cmd.name)
    ...
    def from_CmdBase_instance_method(cmd_method, args):
        ...
        def executor(input, cmdproto):
            ...
            return cmd_method(' '.join([args, input]).strip())
